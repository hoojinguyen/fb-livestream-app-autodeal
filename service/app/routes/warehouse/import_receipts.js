...
const dbReceiptDetail = db.ImportReceiptDetail;
const dbReceipt = db.ImportReceipt;
const dbVariant = db.Variant;
const dbProduct = db.Product;
const nameReceipt = "Receipt Import";
const nameReceiptDetails = "Receipt Import Details";
...
const plusStock = async ({ variantId, quantity }) => {
...
module.exports = {
	attach(router) {
		router.get("/warehouse/import", ctx =>
			protectedRoute("user", getReceipts, ctx)
		);
		router.get("/warehouse/import/:id", ctx =>
			protectedRoute("user", getReceiptById, ctx)
		);
		router.post("/warehouse/import", ctx =>
			protectedRoute("user", createReceipt, ctx)
		);
		router.put("/warehouse/import", ctx =>
			protectedRoute("user", updateReceipt, ctx)
		);
		router.delete("/warehouse/import", ctx =>
			protectedRoute("user", destroyReceipt, ctx)
		);
		router.delete("/warehouse/import/hard", ctx =>
			protectedRoute("admin", hardDeleteReceipt, ctx)
		);
		router.get("/warehouse/import/details/:id", ctx =>
			protectedRoute("user", getDetailsByReceiptId, ctx)
		);
		router.post("/warehouse/import/details", ctx =>
			protectedRoute("user", createDetails, ctx)
		);
		router.put("/warehouse/import/details", ctx =>
			protectedRoute("user", updateDetails, ctx)
		);
		router.delete("/warehouse/import/details", ctx =>
			protectedRoute("user", destroyDetails, ctx)
		);
		router.delete("/warehouse/import/details/hard", ctx =>
