  ...
  <div class="warehouse-import-container">
    ...
    <div class="receipts-wrapper">
      ...
      >
        ...
        <Column field="createdAt" header="Ngày nhập">
          <template #body="slotProps">
            <span>{{ slotProps.data.createdAt | formatDate() }}</span>
          </template>
        </Column>
        <Column field="code" header="Số phiếu nhập"></Column>
        ...
        <template #expansion="slotProps">
          ...
          >
            ...
            >
              <template #header>
                <div class="table-header">
                  <span class="title-detail">Chi tiết phiếu nhập</span>
                </div>
              </template>
    ...
    >
      ...
      <div class="p-fluid p-formgrid p-grid">
        <div class="p-field p-col-4">
          <label for="codeReceipt">Số phiếu nhập</label>
          <InputText
            id="codeReceipt"
          ...
          <small class="p-invalid" v-if="submitted && !receipt.code"
            >Số phiếu nhập không hợp lệ.</small
          >
        </div>
        <div class="p-field p-col-4">
          ...
          <Dropdown
            ...
            :options="instants"
            optionLabel="title"
            placeholder="Đối tượng nhập kho"
            autofocus
          />
        </div>
        <div class="p-field p-col-4">
...
export default {
  ...
  computed: {
    ...mapState({
      receipts: (state) => state.warehouseImport.receipts,
      receiptInStore: (state) => state.warehouseImport.receipt,
      receiptDetails: (state) => state.warehouseImport.receiptDetails,
      statusCreate: (state) => state.warehouseImport.status.create,
      statusUpdate: (state) => state.warehouseImport.status.update,
      statusDelete: (state) => state.warehouseImport.status.delete,
      isLoading: (state) => state.warehouseImport.isLoading,
      isLoadingButton: (state) => state.warehouseImport.isLoadingButton,
      instants: (state) => state.warehouseImport.instants,
  ...
  methods: {
    ...mapActions({
      getReceipts: "warehouseImport/fetchReceipts",
      getReceipt: "warehouseImport/fetchReceiptById",
      createReceipt: "warehouseImport/createReceipt",
      createDetails: "warehouseImport/createDetails",
      updateReceipt: "warehouseImport/updateReceipt",
      updateDetails: "warehouseImport/updateDetails",
      deleteReceipt: "warehouseImport/deleteReceipt",
      deleteDetails: "warehouseImport/deleteDetails",
      getInstants: "warehouseImport/fetchInstants",
      getReceiptDetails: "warehouseImport/fetchDetails",
    ...
    openNew() {
      this.titleDialog = "Thêm mới phiếu nhập kho";
      this.receipt = deepClone(this.receiptDefault);
      this.submitted = false;
    ...
    async openEdit(receipt) {
      ...
      this.titleDialog = "Cập nhật phiếu nhập kho";
    ...
    async printReport(receipt) {
      ...
      let title = `<h3>Phiếu nhập kho</h3>`;
